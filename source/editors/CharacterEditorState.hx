	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			if(!ClientPrefs.OldHDbg) {
				...
				var bg:BGSprite = new BGSprite('stageback-old', -1800 + OFFSET_X - playerXDifference, -1200, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 0.5));
				bgLayer.add(bg);

				var stageFront:BGSprite = new BGSprite('stagefront-old', -950 + OFFSET_X - playerXDifference, 600, 0.98, 0.98);
	...
	function reloadCharacterDropDown() {
		...
		for(mod in Paths.getGlobalMods())
			if(!ClientPrefs.OldHDbg) {
				...
				directories.push(Paths.mods(mod + '/characters old/'));
			}
