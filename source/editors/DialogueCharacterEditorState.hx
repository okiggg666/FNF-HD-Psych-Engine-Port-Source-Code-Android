...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	var invisibleCheckbox:FlxUICheckBox;
	function addTypeUI() {
		...
		invisibleCheckbox = new FlxUICheckBox(rightCheckbox.x, rightCheckbox.y + 40, null, null, "Invisible", 100);
		invisibleCheckbox.callback = function()
		{
			character.jsonFile.dialogue_pos = 'invisible';
			updateCharTypeBox();
		};
		...
		tab_group.add(invisibleCheckbox);
