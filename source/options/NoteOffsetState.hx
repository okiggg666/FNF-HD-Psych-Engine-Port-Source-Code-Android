...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(!ClientPrefs.OldHDbg) {
			...
			if(!ClientPrefs.lowQuality) {
				...
				var stageCurtains:BGSprite = new BGSprite('stagecurtains', -309, -108, 1.3, 1.3);
				stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.47));
				stageCurtains.updateHitbox();
				add(stageCurtains);

				freshCrowd = new BGSprite('CROWD_DADBATTLE', -191, 520, 1.3, 1.3, ['crowd 2']);
			...
			if(!ClientPrefs.lowQuality) {
				...
				var stageCurtains:BGSprite = new BGSprite('stagecurtains-old', -300, -200, 1.3, 1.3);
				stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.47));
				stageCurtains.updateHitbox();
				add(stageCurtains);
	...
	override public function beatHit()
	{
		...
		if(curBeat % 4 == 2)
		{
			...
			if(zoomTween != null) zoomTween.cancel();
			zoomTween = FlxTween.tween(FlxG.camera, {zoom: 0.90}, 1, {ease: FlxEase.circOut, onComplete: function(twn:FlxTween)
				{
					zoomTween = null;
				}
			});
