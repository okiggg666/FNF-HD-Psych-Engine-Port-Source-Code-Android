...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "playRandomSound", function(sound:String, min:Int = 1, max:Int = 1, volume:Float = 1, ?tag:String = null) {
			if(tag != null && tag.length > 0) {
				tag = tag.replace('.', '');
				if(PlayState.instance.modchartSounds.exists(tag)) {
					PlayState.instance.modchartSounds.get(tag).stop();
				}
				PlayState.instance.modchartSounds.set(tag, FlxG.sound.play(Paths.soundRandom(sound, min, max), volume, false, function() {
					PlayState.instance.modchartSounds.remove(tag);
					PlayState.instance.callOnLuas('onRandomSoundFinished', [tag]);
				}));
				return;
			}
			FlxG.sound.play(Paths.soundRandom(sound, min, max), volume);
		});
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			...
			case 'camoverlay' | 'overlay': return PlayState.instance.camOverlay;
